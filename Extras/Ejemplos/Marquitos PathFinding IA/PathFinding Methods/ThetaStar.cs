        ...
        )
    {
        ...
        while (!open.IsEmpty)//Todavia haya nodos para chequear
        {
            ...
            foreach (var childPair in expand(current))
            {
                ...
                if (!inSight(current, child)) continue;
                ...
                {
                    //Tienes que comparar el costo total del actual al nuevo nodo
                    //Vs el peso del abuelo al nuevo nodo.
                    //SI el abuelo puede conectarse al nuevo nodo.

                    var temptativeCost = currentCost + cost(current, child); //Costo total.

                    //Vamos por el abuelo, Acá deberíamos chequear que esten conectados!
                    var grandParent = parents[current];
                    if (grandParent != null && inSight(grandParent, child))
                    {
                        var alternativeCost = costs[grandParent] + cost(grandParent, child);
                        if (alternativeCost < temptativeCost)
                        {
                            parents[child] = grandParent;
                            costs[child] = alternativeCost;
                            open.Enqueue(child, alternativeCost + heuristic(child));//Lo agregamos a la cola
                            continue;
                        }
                    }

                    parents[child] = current;
                    costs[child] = temptativeCost;
                    open.Enqueue(child, temptativeCost + heuristic(child));//Lo agregamos a la cola
                }
                else
                {
                    //Normal
